...
exports.experimentDecorator = void 0;
...
const experimentDecorator = async xml => {
  const data = (await (0, _utils.createXml2jsPromise)(xml)).Response;
  ...
  if (Workflow) {
    type = {
      workflow: parseInt(Workflow[0].Key[0])
    };
    runsData = Workflow[0].Runs[0].Run;
  } else if (Service) {
    type = {
      service: parseInt(Service[0].Key[0])
    };
...
exports.experimentDecorator = experimentDecorator;