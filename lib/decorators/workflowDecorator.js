...
exports.workflowDecorator = void 0;
...
const workflowDecorator = async (xmlFile, webservices) => {
  ...
  for (const step of steps.Step) {
    ...
    const currentWebservice = webservices.find(({
      id
    }) => id == stepKey);
    const {
    ...
    const service = {
      stepName,
      no: stepNo,
      inputs: JSON.parse(JSON.stringify(serviceInputs)),
      // deep clone, otherwise it shares inputs with same named services
      name: serviceName,
      outputs: serviceOutputs,
      description: serviceDescription,
      expectedRuntime,
    ...
    if (Parameter) {
      for (const input of Parameter) {
        ...
        const serviceInput = service.inputs.find(({
          name
        }) => name == pName);
...
exports.workflowDecorator = workflowDecorator;