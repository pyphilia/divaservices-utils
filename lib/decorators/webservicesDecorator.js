...
exports.webservicesDecorator = exports._serviceDecorator = exports.serviceDecorator = void 0;
...
const buildDataInformation = data => {
  if (data.MimeTypes) {
    const {
      MimeTypes: [mimeTypes]
    } = data; // handle file and folder cases

    const type = mimeTypes.Default[0] === _constants.Types.FOLDER.type ? _constants.Types.FOLDER.type : _constants.Types.FILE.type;
    return {
      type,
    ...
    return {
      defaultValue,
      values: Value,
      type: _constants.Types.SELECT.type
    };
    ...
    const {
      ...
      Max,
      Step
    } = data.Type[0].StepNumberType[0];
    const res = {
      type: _constants.Types.NUMBER.type,
      values: {}
    };
...
const serviceDecorator = async xml => {
  const json = await (0, _utils.createXml2jsPromise)(xml);
  return _serviceDecorator(json.Service);
};

exports.serviceDecorator = serviceDecorator;

const _serviceDecorator = xml => {
  const {
    Id: [id],
  ...
  if (Data) {
    for (const paramData of Data) {
      ...
      const {
        ...
        allowed,
        defaultValue
      } = buildDataInformation(paramData);
      const param = {
        description,
        name,
        type,
  ...
  if (Parameter) {
    for (const parameter of Parameter) {
      ...
      const {
        ...
        defaultValue,
        values
      } = buildDataInformation(parameter);
      const param = {
        description,
        name,
        type,
  ...
  if (Text) {
    for (const text of Text) {
      ...
      const param = {
        description,
        name,
        type: _constants.Types.TEXT.type
      };
...
exports._serviceDecorator = _serviceDecorator;

const webservicesDecorator = async xmlFile => {
...
exports.webservicesDecorator = webservicesDecorator;